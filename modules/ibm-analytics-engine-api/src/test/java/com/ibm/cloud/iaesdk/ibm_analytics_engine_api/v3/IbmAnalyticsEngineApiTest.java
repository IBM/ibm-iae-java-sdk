...
public class IbmAnalyticsEngineApiTest extends PowerMockTestCase {
  ...
  */
  ...
  public void testDisablePlatformLoggingWOptions() throws Throwable {
    // Schedule some responses.
    String mockResponseBody = "{\"components\": [\"components\"], \"log_server\": {\"type\": \"ibm-log-analysis\"}, \"enable\": false}";
    ...
    // Invoke operation with valid options model (positive test)
    Response<LoggingDisableResponse> response = ibmAnalyticsEngineApiService.disablePlatformLogging(disablePlatformLoggingOptionsModel).execute();
    assertNotNull(response);
    LoggingDisableResponse responseObj = response.getResult();
    assertNotNull(responseObj);
